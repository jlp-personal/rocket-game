...
class Game {
    ...
    selectDestination(key) {
        ...
        if (minCrew)
            minCrew.textContent = destination.minCrew.toString();
        if (recFuel)
            recFuel.textContent = destination.recommendedFuel.toString();
        if (reward)
            reward.textContent = destination.reward.toLocaleString();
    ...
    selectRocket(index) {
        ...
        this.currentRocket = new Rocket(config.name, config.fuelCapacity, config.maxCrewCapacity, config.fuelConsumptionRate);
        this.updateBudgetDisplay();
    ...
    enableFuelControls() {
        ...
        if (fuelPreview)
            fuelPreview.classList.add('hidden');
        if (fuelControls)
            fuelControls.classList.remove('hidden');
    }
    addFuel(amount) {
    ...
    attemptLaunch() {
        ...
        if (crewCount < destination.minCrew) {
            this.showMessage(`Not enough crew for ${destination.name}! Need ${destination.minCrew}, have ${crewCount}`, 'error');
            return;
        }
        try {
    ...
    updateAllDisplays() {
        ...
        if (rocketName)
            rocketName.textContent = this.currentRocket.getName();
        ...
        if (fuelCurrentMain)
            fuelCurrentMain.textContent = fuelCurrent.toFixed(0);
        if (fuelMaxMain)
            fuelMaxMain.textContent = fuelMax.toFixed(0);
        if (fuelPercentMain)
            fuelPercentMain.textContent = fuelPercent.toFixed(1);
        if (fuelLevelMain)
            fuelLevelMain.style.width = `${fuelPercent}%`;
        // Rocket status fuel
        ...
        if (fuelCurrentStatus)
            fuelCurrentStatus.textContent = fuelCurrent.toFixed(0);
        if (fuelMaxStatus)
            fuelMaxStatus.textContent = fuelMax.toFixed(0);
        if (fuelPercentStatus)
            fuelPercentStatus.textContent = fuelPercent.toFixed(1);
        if (fuelLevelStatus)
            fuelLevelStatus.style.width = `${fuelPercent}%`;
        ...
        if (crewCountMain)
            crewCountMain.textContent = crew.length.toString();
        if (crewMaxMain)
            crewMaxMain.textContent = crewMax.toString();
        if (crewListMain) {
            if (crew.length === 0) {
                crewListMain.innerHTML = '<p class="no-crew">No crew members hired yet</p>';
            }
            else {
                crewListMain.innerHTML = crew.map(member => `<div class="crew-member">👨‍🚀 ${member}</div>`).join('');
            }
        }
        // Rocket status crew
        ...
        if (crewCountStatus)
            crewCountStatus.textContent = crew.length.toString();
        if (crewMaxStatus)
            crewMaxStatus.textContent = crewMax.toString();
        if (crewListStatus) {
            if (crew.length === 0) {
                crewListStatus.innerHTML = '<p class="no-crew">No crew members assigned</p>';
            }
            else {
                crewListStatus.innerHTML = crew.map(member => `<div class="crew-member">👨‍🚀 ${member}</div>`).join('');
            }
        }
    }
    updateBudgetDisplay() {
